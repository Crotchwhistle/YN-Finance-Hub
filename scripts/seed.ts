...
const SEED_USER_ID = "user_2qpmh5DhcgELEPrRill7PjfML6Q";
...
const generateRandomAmount = (category: typeof categories.$inferInsert) => {
    switch (category.name) {
        case "Rent":
            return Math.round(Math.random() * 400 + 90)
        case "Utilities":
...
const generateTransactionsForDay = (day: Date) => {
    ...
    for (let i = 1; i < numTransactions; i++) {
        ...
        const formattedAmount = Math.round(convertAmountFromMiliunits(isExpense ? -amount : amount))
